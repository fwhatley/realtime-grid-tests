...
export function getQuoteGridData(price: number = 1.0) {
  return stockSymbols.slice(0, 10).map((symbol) => createQuote(symbol, price))
}

export function transformDataToAddIdsToEachCell(quoteData = [] ) {
    return quoteData.map((row, rowIndex) => {
